...
const uint8_t DEFAULT_DXL_ID = 25; // ID 
const float DXL_PROTOCOL_VERSION = 2.0;
const uint32_t DEFAULT_BAUDRATE = 115200; // Baudrate awal, set port baudrate to match with DYNAMIXEL baudrate.
...
void setup() {
  uint32_t new_baudrate = 1000000; // Baudrate baru
  ...
  DEBUG_SERIAL.print(": ");
  if(dxl.ping(DEFAULT_DXL_ID) == true) {
    
    DEBUG_SERIAL.print("ping succeeded!");
    DEBUG_SERIAL.print(", Model Number: ");
    DEBUG_SERIAL.println(dxl.getModelNumber(DEFAULT_DXL_ID));
    
    // Turn off torque when configuring items in EEPROM area
    dxl.torqueOff(DEFAULT_DXL_ID);

    if (dxl.setBaudrate(DEFAULT_DXL_ID, new_baudrate)) {
    DEBUG_SERIAL.print("Baud rate set to: ");
    DEBUG_SERIAL.println(new_baudrate);
    } else {
    DEBUG_SERIAL.println("Failed to change baud rate :(((");
    }
  }
  else{
